...
public class SplitFileHasher implements FileHasher {
    ...
    public SplitFileHasher(FileHasher globalHasher, FileHasher localHasher, FileCategorizer fileCategorizer) {
    ...
    public HashCode hash(FileTreeElement fileDetails) {
        if (fileCategorizer.isImmutable(fileDetails.getPath())) {
